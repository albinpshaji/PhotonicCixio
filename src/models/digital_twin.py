...
class PhotonicMeshDigitalTwin(nn.Module):
    ...
    def compute_transfer_matrix(
        ...
        theta_phys, phi_phys = self.prepare_physical_phases(theta, phi)
        ...
        for layer in self.layer_info:
            ...
            if self.ideal_mode or not self.config.enable_coupler_errors:
                eps1_col = None
                eps2_col = None
                phi_int_col = None
            else:
            ...
            # Apply progressive stage insertion loss
    ...
    def propagate_field(
        ...
        for layer in self.layer_info:
            ...
            # Apply inter-column waveguide crossings
            if self.config.enable_physical_routing and not self.ideal_mode:
                if use_pol:
    ...
    def forward(
        ...
        if E_in is None:
            ...
            return self.compute_transfer_matrix(theta=theta, phi=phi, wavelength=wavelength)

        # Propagate optical field through Clements mesh
        E_out = self.propagate_field(E_in=E_in, theta=theta, phi=phi, wavelength=wavelength)
        ...
        is_balanced = balanced if balanced is not None else self.config.enable_balanced_detection
        if is_balanced and not self.ideal_mode and self.n_modes % 2 == 0:
            # Balanced detection: pairs (2k, 2k+1)
            if use_pol:
        ...
        if use_pol:
            ...
            I_out = self.photodetector(E_out=E_eff, add_noise=add_noise and not self.ideal_mode)
        else:
            I_out = self.photodetector(E_out=E_out, add_noise=add_noise and not self.ideal_mode)
